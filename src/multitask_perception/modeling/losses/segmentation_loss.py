...
# NOTE:Implementation of ESPNetv2 Segmentation Loss in Multitask Framework
# ------------------------------------------------------------------------------
import torch
...
class SegmentationLoss(nn.Module):
    def __init__(
        ...
        if self.loss_type == "bce":
            ...
            self.loss_fn = nn.CrossEntropyLoss(
                ignore_index=self.ignore_idx, weight=self.class_wts
            )
    ...
    def forward(self, inputs, target):
        if isinstance(target, list):
            target = (
                torch.stack(target).cuda()
                if torch.cuda.is_available()
                else torch.stack(target)
            )
