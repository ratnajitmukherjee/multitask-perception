...
def do_train(cfg, model,
    ...
    with EnergyMeter(writer=summary_writer, dir=checkpointer.save_dir) if args.calc_energy else ExitStack():
        for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
            ...
            if args.eval_step > 0 and iteration % args.eval_step == 0 and not iteration == max_iter:
                inf_time, eval_results = do_evaluation(cfg, model, distributed=args.distributed,
